...
abstract class BotTransportAbstract
{
	...
	public function sendCallbackResult($text)
	{
		...
		$response->set("reply_markup", ['remove_keyboard' => true]);
	...
	{
//	    BotLog::log("1Method: ".$response->getMethod() . "; ". json_encode($response->getArray()));
		return $this->send($response->getMethod(), $response);
	}
	
	public function send($methodParam, $response){
//		BotLog::log("2Method: ".$methodParam . "; ". json_encode($response->getArray()));
		$method = $methodParam;
		/*
		...
		$request = json_encode($this->getRequest()->getRequestData());
		$this->log("method=$method;data=$data;req=$request;url=$url;res=$result");
	...
	public function log($data)
	{
		...
		file_put_contents(PATH_PROJECT . '/bot-transport.txt', print_r($data, 1) . "\n", FILE_APPEND);
	}
}
