...
//		$this->resText = $this->getWrapped("res")->getWrapped("text")->getWrapped($this->getLang());
	...
	public function getContentConfig(): ArrayWrapper{
		$content = $this->getResource()->getWrapped("content");
		return $content;
	}
	public function getContentConfigForGroup($groupId){
		$content = $this->getContentConfig();
		return $content->getWrapped("$groupId");
	}
	public function getContentConfigForGroupLang($groupId){
		$content = $this->getContentConfig();
		return $content->getWrapped("$groupId")->getWrapped($this->getLang());
	}
