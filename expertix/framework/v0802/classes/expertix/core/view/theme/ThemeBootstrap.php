...
class ThemeBootstrap extends ThemeHtml{
...
EOT);
	...
	function title($title, $h=2, $class="mt-3")
	{
		$title = "<div class='title$h block-title $class'>$title</div>";
		return $this->process(<<<EOT
$title
EOT);
	}	
	function card($title, $subTitle, $img, $link, $body_class = "bg-white text-black", $link_text = "Перейти", $footer = null)
	{
		$link_html = $link ? "<a href='$link' class='card-link stretched-link'></a>" : "";
