...
class ApiAuthByLoginController extends BaseApiController
{
	 ...
	 */
	 ...
	 public function signUp($request){
		...
		if ($request->get("email_after_signup")) {
			$this->notifyUser($request, $user);
		}
		return $user;
	 }

	protected function notifyAdmin($request, $user)
	{
		Log::d("notifyAdmin", $user);
		$emailController = new UserMessageManager();
		return $emailController->notifyAdminOnSignup($user);
	}
	protected function notifyUser($request, $user)
	{
		$emailController = new UserMessageManager();
		return $emailController->notifyUserOnSignup($user);
	}
	...
	public function signInByLoginAndPassword($request)
	{
		...
		$password = $request->getRequired("password", "Необходимо указать пароль");
		$authManager = $this->getUserManager();

		$user = $authManager->authByPassword($login, $password);
