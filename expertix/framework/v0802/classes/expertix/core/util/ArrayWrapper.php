...
class ArrayWrapper{
	...
	public function getAndCheckInRange($key, $from, $to = null, $default=null, $errorMessage=null){
		$value = $this->get($key);
		if(!$value){
			return $default;
		}
		
		if($this->checkInRange($value, $from, $to)){
			return $value;
		}else{
			if($default){
				return $default;
			}else{
				throw new \Exception($errorMessage  or "Value $value is not in range");
			}
		}
	}
	
	private function checkInRange($value, $from, $to = null){
		if(!$to){
			return $value == $from;
		}
