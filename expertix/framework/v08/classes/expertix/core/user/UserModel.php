...
class UserModel extends BaseModel
{
	...
	//	$sql = "select u.*, p.* from app_user u left join app_person p on u.personId=p.personId where authLink='nimbda'";
		$user = DB::getRow($sql, [$link]);
		return $user;		
	}
	public function getUserByLogin($login)
	{
		$sql = self::SQL_SELECT1." and login=?";
		return DB::getRow($sql, [$login]);
	}
	...
	public function getUsersForCompany($companyId)
	{
		...
		return DB::getRow($sql, [$companyId]);
	}
	...
	public function getUsersTest()
	{
		...
		for ($i = 0; $i < 10000; $i++) {
			$result[] = [
				"firstName" => "$i dsfdsgh lghdfj ghfjkgh dfg hdfjkgh dfjkgdsfjkg hdfgjhdsfjkg hdfjk gjk jhg djfk gdfjk gdfjk g",
				"lastName" => "$i dsfdsgh lghdfj ghfjkgh dfg hdfjkgh dfjkgdsfjkg hdfgjhdsfjkg hdfjk gjk jhg djfk gdfjk gdfjk g",
